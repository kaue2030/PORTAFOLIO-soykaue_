    ...
    <main className="min-h-screen bg-[#f5f4f3] text-black overflow-hidden selection:bg-[#a3e635]/30">
      ...
      <div className="fixed top-6 left-4 md:left-8 z-[60]">
        <Link href={`/ensenanzas/${resolvedParams.slug}/deidad`} className="flex items-center gap-2 text-gray-400 hover:text-black transition-colors group">
          ...
          <span className="font-medium text-sm md:text-base">{t.lessons.backTo} {t.lessons.deityOfChrist}</span>
        </Link>
      </div>
...
function CristologiaEncarnacion() {
  ...
  const c = cristologiaContent[language].encarnacion;
    ...
    <article className="space-y-12 md:space-y-16 pb-20">
      ...
      <div className="space-y-10 text-black/80 text-sm md:text-base leading-relaxed">
        <section className="space-y-4">
          {isEditorMode ? (
            <input value={editContent.intro_title} onChange={(e) => setEditContent({...editContent, intro_title: e.target.value})} className="text-xl md:text-2xl font-bold text-black bg-white border border-black/10 rounded w-full p-2" />
          ) : (
            <h2 className="text-xl md:text-2xl font-bold text-black">{current.intro_title}</h2>
          )}

          {isEditorMode ? (
            <textarea value={typeof editContent.intro_p1 === 'string' ? editContent.intro_p1 : 'Complex content'} onChange={(e) => setEditContent({...editContent, intro_p1: e.target.value})} className="w-full h-24 p-4 bg-white border border-black/10 rounded-xl" />
          ) : (
            <div className="text-justify">{current.intro_p1}</div>
          )}

          {isEditorMode ? (
            <textarea value={typeof editContent.intro_p2 === 'string' ? editContent.intro_p2 : 'Complex content'} onChange={(e) => setEditContent({...editContent, intro_p2: e.target.value})} className="w-full h-24 p-4 bg-white border border-black/10 rounded-xl" />
          ) : (
            <div className="text-justify">{current.intro_p2}</div>
          )}
        ...
        <section className="space-y-4">
          {isEditorMode ? (
            <input value={editContent.before_title} onChange={(e) => setEditContent({...editContent, before_title: e.target.value})} className="text-xl md:text-2xl font-bold text-black bg-white border border-black/10 rounded w-full p-2 text-right" />
          ) : (
            <h2 className="text-xl md:text-2xl font-bold text-black text-right">{current.before_title}</h2>
          )}

          {isEditorMode ? (
            <textarea value={typeof editContent.before_p1 === 'string' ? editContent.before_p1 : 'Complex content'} onChange={(e) => setEditContent({...editContent, before_p1: e.target.value})} className="w-full h-24 p-4 bg-white border border-black/10 rounded-xl" />
          ) : (
            <div className="text-justify">{current.before_p1}</div>
          )}
        ...
        <section className="space-y-4">
          {isEditorMode ? (
            <input value={editContent.after_title} onChange={(e) => setEditContent({...editContent, after_title: e.target.value})} className="text-xl md:text-2xl font-bold text-black bg-white border border-black/10 rounded w-full p-2 text-right" />
          ) : (
            <h2 className="text-xl md:text-2xl font-bold text-black text-right">{current.after_title}</h2>
          )}

          {isEditorMode ? (
            <textarea value={typeof editContent.after_p1 === 'string' ? editContent.after_p1 : 'Complex content'} onChange={(e) => setEditContent({...editContent, after_p1: e.target.value})} className="w-full h-24 p-4 bg-white border border-black/10 rounded-xl" />
          ) : (
            <div className="text-justify">{current.after_p1}</div>
          )}

          {isEditorMode ? (
            <textarea value={typeof editContent.after_p2 === 'string' ? editContent.after_p2 : 'Complex content'} onChange={(e) => setEditContent({...editContent, after_p2: e.target.value})} className="w-full h-24 p-4 bg-white border border-black/10 rounded-xl" />
          ) : (
            <div className="text-justify">{current.after_p2}</div>
          )}
          ...
          {isEditorMode ? (
            <textarea value={editContent.after_p3} onChange={(e) => setEditContent({...editContent, after_p3: e.target.value})} className="w-full h-20 p-4 bg-white border border-black/10 rounded-xl" />
          ) : (
            <p>{current.after_p3}</p>
          )}
        ...
        <section className="space-y-4">
          {isEditorMode ? (
            <input value={editContent.full_title} onChange={(e) => setEditContent({...editContent, full_title: e.target.value})} className="text-xl md:text-2xl font-bold text-black bg-white border border-black/10 rounded w-full p-2 text-right" />
          ) : (
            <h2 className="text-xl md:text-2xl font-bold text-black text-right">{current.full_title}</h2>
          )}

          {isEditorMode ? (
            <input value={editContent.full_p1} onChange={(e) => setEditContent({...editContent, full_p1: e.target.value})} className="w-full p-2 bg-white border border-black/10 rounded" />
          ) : (
            <p>{current.full_p1}</p>
          )}

          <ol className="list-decimal ml-6 space-y-1 font-medium">
            <li><span className="font-bold">{current.full_l1}</span></li>
            <li><span className="font-bold">{current.full_l2}</span></li>
          </ol>

          {isEditorMode ? (
            <textarea value={typeof editContent.full_p2 === 'string' ? editContent.full_p2 : 'Complex content'} onChange={(e) => setEditContent({...editContent, full_p2: e.target.value})} className="w-full h-24 p-4 bg-white border border-black/10 rounded-xl" />
          ) : (
            <div className="text-justify">{current.full_p2}</div>
          )}
        ...
        <section className="space-y-4">
          {isEditorMode ? (
            <input value={editContent.dual_title} onChange={(e) => setEditContent({...editContent, dual_title: e.target.value})} className="text-xl md:text-2xl font-bold text-black bg-white border border-black/10 rounded w-full p-2 text-right" />
          ) : (
            <h2 className="text-xl md:text-2xl font-bold text-black text-right">{current.dual_title}</h2>
          )}

          {isEditorMode ? (
            <textarea value={typeof editContent.dual_p1 === 'string' ? editContent.dual_p1 : 'Complex content'} onChange={(e) => setEditContent({...editContent, dual_p1: e.target.value})} className="w-full h-24 p-4 bg-white border border-black/10 rounded-xl" />
          ) : (
            <div className="text-justify">{current.dual_p1}</div>
          )}
