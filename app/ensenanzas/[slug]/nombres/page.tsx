    ...
    <main className="min-h-screen bg-[#f5f4f3] text-black overflow-hidden selection:bg-[#a3e635]/30">
      ...
      <div className="fixed top-6 left-4 md:left-8 z-[60]">
        <Link href={`/ensenanzas/${resolvedParams.slug}/encarnacion`} className="flex items-center gap-2 text-gray-400 hover:text-black transition-colors group">
          ...
          <span className="font-medium text-sm md:text-base">{t.lessons.backTo} {t.lessons.incarnationOfChrist}</span>
        </Link>
      </div>
...
function CristologiaNombres() {
  ...
  const c = cristologiaContent[language].nombres;
  ...
  return (
    <article className="space-y-12 md:space-y-16 pb-20">
      ...
      <div className="space-y-10 text-black/80 text-sm md:text-base leading-relaxed">
        ...
        <section className="space-y-4">
          {isEditorMode ? (
            <input value={editContent.s4_title} onChange={(e) => setEditContent({...editContent, s4_title: e.target.value})} className="text-xl md:text-2xl font-bold text-black bg-white border border-black/10 rounded w-full p-2 text-right underline underline-offset-4" />
          ) : (
            <h2 className="text-xl md:text-2xl font-bold text-black text-right underline underline-offset-4">{current.s4_title}</h2>
          )}
